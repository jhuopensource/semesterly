    ...
    def handle(self, *args, **options):
        ...
        with CommandLogger("deleteinactiveuser", log_args) as cmd_logger:
            try:
                ...
                inactive_users_pks = User.objects.filter(
                    last_login__lte=cutoff_date
                ).values_list("pk")[:batch_size]
                ...
                cmd_logger.log_output(
                    f"Found {count} inactive users (last login before {cutoff_date})."
                )
                ...
                if dry_run:
                    ...
                    for user in inactive_users:
                        cmd_logger.log_output(
                            f"Would delete: {user.username} (last login: {user.last_login})"
                        )
                else:
                    # Log the users that will be deleted
                    for user in inactive_users:
                        logger.info(
                            f"Deleting inactive user: {user.username} (last login: {user.last_login})"
                        )
                        cmd_logger.log_output(
                            f"Deleting user: {user.username} (last login: {user.last_login})"
                        )

                    # Delete the users
                    inactive_users.delete()
                    cmd_logger.log_output(
                        f"Successfully deleted {count} inactive users."
                    )
